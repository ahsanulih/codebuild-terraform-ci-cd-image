...
def display_cost_estimation(not_free_resource_counter, free_resource_counter, not_supported_resource_counter,
    ...
    if not_free_resource_counter == 0 and free_resource_counter == 0 and not_supported_resource_counter == 0:
        ...
        if not_free_resource_counter == 1:
            print(f"∙ {not_free_resource_counter} was estimated. Total running cost of 30 days: ${total_cost:.2f}")
        elif not_free_resource_counter > 1:
            print(f"∙ {not_free_resource_counter} were estimated. Total running cost of 30 days: ${total_cost:.2f}")
    ...
    try:
        data = pd.read_csv('/usr/local/bin/infra-cost-estimator/data/data.csv', skiprows = 1)
    except:
